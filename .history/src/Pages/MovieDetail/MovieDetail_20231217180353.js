...
const MovieDetail = props => {
  ...
  useEffect(() => {
    const getPeople = async () => {
      const data = await GET(`/movie/${props.route.params.movieId}/credits`);
      setPeople(data);
      setLoading(false);
    };
  ...
  const getCast = () => {
    return people.casts.map(cast => (
      <View style={Styles.trendingPeopleContainer}>
      <Image
        source={{uri: `${IMAGE_POSTER_URL}${cast.profile_path}`}}
        style={Styles.trendingPeopleImage}
      />
      <Text style={Styles.trendingPeopleName}>{cast.name}</Text>
      <Text style={Styles.trendingPeopleCharacter}>{cast.character}</Text>
      <Text style={Styles.trendingPeopleCharacter}>{cast.job}</Text>
    </View>
    ));
  };
